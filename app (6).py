...
# Sliders for inputs
A = st.slider("Area of reservoir (acres)", min_value=1.0, max_value=10000.0, value=500.0, step=10.0)
...
if reserve_type == 'Oil':
    Bo = st.slider("Formation Volume Factor Bo (RB/STB)", min_value=1.0, max_value=2.5, value=1.2, step=0.01)
else:
    Bg = st.slider("Formation Volume Factor Bg (RCF/SCF)", min_value=0.0001, max_value=0.02, value=0.005, step=0.0001)

# Optional Recovery Factor
RF = st.slider("Recovery Factor (optional)", min_value=0.0, max_value=1.0, value=0.3, step=0.01)
...
if RF > 0:
    ...
    unrecovered = total_in_place - recoverable

    fig1, ax1 = plt.subplots()

    def format_autopct(pct):
        absolute = int(round(pct / 100. * total_in_place))
        return f"{pct:.1f}%\n({absolute:,})"
